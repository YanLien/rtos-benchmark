...
COMPILE_C "src/common/bench_utils.c"               "${BD}/bench_utils.o"        || ERRORS=$((ERRORS+1))
COMPILE_C "src/common/bench_all.c"                 "${BD}/bench_all.o"          || ERRORS=$((ERRORS+1))
...
for test in thread malloc_free message_queue mutex_lock_unlock \
    ...
    if [ -f "src/common/bench_${test}_test.c" ]; then
        COMPILE_C "src/common/bench_${test}_test.c" "${BD}/bench_${test}_test.o" || ERRORS=$((ERRORS+1))
    fi
done
